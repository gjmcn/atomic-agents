...
| `autotile(squares,`<br>&emsp;`options)` | Assign a tile to each square in the iterable `squares`. Returns a `{complete, assignments, attempts, backtracks}` object. `attempts` and `backtracks` indicate the total number of attempts and backtracks used. If a solution is found, `complete` is `true` and assignments is a map where each key is a square, and the value is a `{name, rotationCode}` object containing the name of the tile assigned to the square, and the tile's rotation: `0` none, `1` 90° clockwise, `2` 180°, `3` 270°. If no solution is found, `complete` is `false` and the `assignments` map contains the  assignments made on the last attempt. Square-tile pairs are added to the `assignments` map in the order they are chosen. `options` is an object with properties: <ul style="margin:0"><li>`tiles`: object. Each key is a name; the value is an object with properties:<ul style="margin:0"><li>`edges`: an edge name (string) or array of edge names (top, right, bottom,  left)</li>  <li>`rotate = false`: specifies if the tile can be rotated.</li><li>`prob = 1`: prior probability of tile. Value should be ≥ 0, but probabilities need not be normalised. If `prob` is a function, it is passed the square the tile is being considered for and returns the probability.</li></ul><li>`edges = {}`: object. Each key is an edge name; the value is the edge name or array of edge names the key can be matched with. If an edge name does not appear as a key, it is assumed the edge can only match itself.</li><li>`startTiles = []`: map (or array of arrays) where each key is a square and the value is a `{ name, rotationCode = 0 }` object. `autotile` finds tiles for all squares not included in startTiles. The 'edge validity' of start tiles is not enforced.</li><li>`retry = 100`: max allowed attempts to find a solution.</li><li>`backtrack = true`: if `true`, when an attempt finds a square with no valid candidate tiles, the algorithm backtracks to the last assigned-to neighbor of the square and chooses a different tile for the neighbor so that the edge facing the no-candidates-square is different.</li></ul> . <br>__Note:__ `autotile` uses a simplified [Wave Function Collapse](https://github.com/mxgmn/WaveFunctionCollapse) approach: the algorithm maintains a list of valid tiles for each square; at each step, a tile is assigned to the square with the lowest entropy (i.e. uncertainty) and the candidate tiles of other squares are updated. This continues until a tile has been assigned to each square.| object |
